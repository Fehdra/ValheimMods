...
namespace InstantMonsterDrop
{
    ...
    public class BepInExPlugin : BaseUnityPlugin
    {
        ...
        private static void Dbgl(string str = "", bool pref = true)
        {
            if (isDebug.Value)
        ...
        {
            static bool Prefix(Ragdoll __instance) => modEnabled.Value;
        }
        ...
        private static IEnumerator DropNow(Ragdoll ragdoll, ZNetView nview, EffectList removeEffect)
        {
            ...
            {
                instance.StartCoroutine(DestroyNow(ragdoll, nview, removeEffect));
                yield break;
            }

            if (!modEnabled.Value || nview == null || !nview.IsValid() || !nview.IsOwner())
                yield break;
        ...
        static class InputText_Patch
        {
            static bool Prefix(Terminal __instance)
            {
                ...
                if (text.Equals($"{typeof(BepInExPlugin).Namespace.ToLower()} reset"))
                {
                    instance.Config.Reload();
                    instance.Config.Save();
                    Traverse.Create(__instance).Method("AddString", text).GetValue();
                    Traverse.Create(__instance).Method("AddString", $"{instance.Info.Metadata.Name} config reloaded").GetValue();
                    return false;
                }
                return true;
            }
